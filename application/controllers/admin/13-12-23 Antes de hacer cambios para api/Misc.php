...

{
    ...
    private function get_provincias_data()
{
    $this->db->select('id, provincias'); // Ajusta los campos según tu esquema
    $provincias = $this->db->get('tblprovincias')->result_array();
    ...
    public function get_relation_data()
{
    if ($this->input->post()) {
        ...
        // Cambia la llamada para obtener datos de provincias
        if ($type == 'provincia' || $type == 'provincias') {
            $data = $this->get_provincias_data(); // Implementa esta función
        } else {
            // Si no es provincia, utiliza la función existente
            $data = get_relation_data($type, '', $this->input->post('extra'));
        }
