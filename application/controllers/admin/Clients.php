        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                if (!has_permission('customers', '', 'create')) {
                  ...
                  $palabraClave = $this->input->post('city'); // Ajusta 'palabra_clave' según el nombre que le hayas dado al campo en tu formulario

                  if (!empty($palabraClave)) {
                      $datosProvincias = $this->clients_model->obtenerDatosProvincias($palabraClave);
                  } else {
                      // Si no se proporciona una palabra clave, obtén todos los datos
                      $datosProvincias = '';
                  }
                  // Accede al primer elemento del array
                  $primerElemento = $datosProvincias[0];
                  //echo '<br><br><br>';
                  //var_dump($datosProvincias);
                  // Accede a los valores específicos
                  $idP = $primerElemento['id'];
                  $nombreProvincia = $primerElemento['provincias'];
                  
                  // Muestra los valores
                  //echo "ID: $id, Provincia: $nombreProvincia";
                  
                  //echo '<br><br><br>';
                  
                  $palabraClaveDistrito = $this->input->post('state'); // Ajusta 'palabra_clave' según el nombre que le hayas dado al campo en tu formulario
                  
                  if (!empty($palabraClaveDistrito)) {
                      $datosDistritos = $this->clients_model->obtenerDatosDistritos($palabraClaveDistrito);
                  } else {
                      // Si no se proporciona una palabra clave, obtén todos los datos
                      $datosDistritos = '';
                  }
                  $primerElementoD = $datosDistritos[0];
                  $idD=$primerElementoD['id'];
                  $distritos=$primerElementoD['distrito'];
                  
                      /*echo '<br><br><br>';
                      var_dump($datosDistritos);*/

                     /*  echo "ID: $idD, Distrito: $distritos";
                  
                  echo '<br><br><br>';*/
                  
                  
                      $palabraClaveCorregimiento = $this->input->post('zip'); // Ajusta 'palabra_clave' según el nombre que le hayas dado al campo en tu formulario
                  
                  if (!empty($palabraClaveCorregimiento)) {
                      $datosCorregimiento = $this->clients_model->obtenerDatosCorregimientos($palabraClaveCorregimiento);
                  } else {
                      // Si no se proporciona una palabra clave, obtén todos los datos
                      $datosCorregimiento = '';
                  }
                  
                  $primerElementoC = $datosCorregimiento[0];
                  $idC= $primerElementoC['id'];
                  $corregimiento=$primerElementoC['corregimiento'];
                      /*echo '<br><br><br>';
                      var_dump($datosCorregimiento);*/
                  
                  
                                  //return;

                $data = $this->input->post();
                // Puedes usar nombres significativos para las claves, por ejemplo:
$data['Corregimientos'] = $idC;
$data['Distritos'] = $idD;
$data['Provincias'] = $idP;
