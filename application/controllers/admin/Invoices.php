...

{
    ...
    public function validate_invoice_number()
    {
        ...
        // Verificar si $date no es null antes de agregar la condición 'YEAR(date)'
        if (!empty($date)) {
            // Solo aplicar strtotime si $date no está vacío
            $timestamp = strtotime(to_sql_date($date));
    
            if ($timestamp !== false) {
                $where_conditions['YEAR(date)'] = date('Y', $timestamp);
            } else {
        ...
        if (total_rows('invoices', $where_conditions) > 0) {
            echo 'false';
        } else {
            echo 'true';
        }
...
// Asegúrate de incluir city, provincias y corregimiento en los datos
$invoice_data['provincia'] = $this->input->post('provincia');
$invoice_data['distrito'] = $this->input->post('distrito');
$invoice_data['corregimiento'] = $this->input->post('corregimiento');
                ...
                if (hooks()->apply_filters('validate_invoice_number', true)) {
                    ...
                    if (!empty($date)) {
                        ...
                        if ($timestamp !== false) {
                            ...
                            // Manejar el caso en que la conversión de fecha falle
                            set_alert('warning', _l('invalid_date_format'));
        ...
        return;*/
            ...
            // Agregar la información del país al array $data
            ...
            $data['cities'] = $this->Main_model->getCity();
             // get Sucursales
             $data['sucursales'] = $this->Main_model->getSucursales();
...
error_reporting(E_ALL);
        ...
        if ($this->input->post()) {
            ...
            //Obtenemos datos de la configuracion de la empresa de nuestro CRM
            $datosCRM=$this->invoices_model->getDateCrm();
            $RucDelEmisor=$datosCRM->value;
            ...
            $TipoIdentificacion=''; 
            $DV= '';
            //Aqui para custom field de clientes
            $datosCustomFieldsClientes = $this->invoices_model->getCustomFieldsValuesClient($datos_del_cliente->userid);
            if (!empty($datosCustomFieldsClientes)) {
                foreach ($datosCustomFieldsClientes as $customField) {
                    if (isset($customField->value)) {
                        $value = $customField->value;
           ...
           /* echo 'TipoC :'.$tipoCliente;
            echo '<br><br><br>';
            echo 'TipoD :'. $TipoIdentificacion;
            echo '<br><br><br>';
            echo 'DV :'.$DV;
            echo '<br><br><br>';
            ...
            //Obtengo el tipo de moneda con la que se opera
            $Moneda= $this->invoices_model->getMoneda($datos_del_invoice->currency);
            $MonedaCode=$Moneda->name;
            /*echo $Moneda->name;
            return;*/
...
$totalgravado=$datos_del_invoice->total_tax;
$BaseImponible=$datos_del_invoice->subtotal;
...
//Armamos el codigo de Ubicacion
$CodigoUbicacion=$datos_del_invoice->provincia."-".$datos_del_invoice->distrito."-".$datos_del_invoice->corregimiento;
...
//Establecimiento que es sucursal
$sucursal=$datos_del_invoice->sucursal;
//punto que es caja
$caja='00'.$datos_del_invoice->caja;
   

//IMPORTANTE SE DEBE BUSCAR DE DONDE SACAR EL RUC DEL EMISOR POR QUE EN EL CONTROLADOR STAFF no esta por ahora estamos colocando este de forma statica 155680264-2-2019
    ...
    //'.$datos_del_cliente->vat.' aca el rut debe ser el de la empresa
        ...
        <Requerimiento>
            ...
            <RucEmisor>'.$RucDelEmisor.'</RucEmisor>
            ...
            <ResumenComprobante>
                <CodigoMoneda>'.$MonedaCode.'</CodigoMoneda>
        ...
        <DigitoVerificador>'.$DV.'</DigitoVerificador>
                <Ubicacion>
                    <DireccionSAT>
...
$this->load->library('ciqrcode'); // Carga la biblioteca

$params['data'] = $decodedQRCode; // Define los datos que quieres codificar en el QR
$params['size'] = 10; // Define el tamaño del QR
$params['savename'] = FCPATH.'assets/files/qr'.$datos_del_invoice->id.'.png'; // Define el nombre del archivo donde se guardará el QR

$this->ciqrcode->generate($params); // Genera el QR

}else{
    echo '<br><br>';
    echo  'Error generado al momento de procesar su pago a DGI, por favor verifcar';
    echo '<br><br>';
}
  ...
  // Después de procesar el pago, puedes obtener el ID de la factura
  /*$invoiceId = 149; // Reemplaza 123 con el ID real de la factura

  // Luego, puedes preparar los datos necesarios para enviar por correo
  $emailData = [
      'attach_statement' => true, // Opcional: establece si se debe adjuntar un estado de cuenta
      'statement_from'   => '2023-01-01', // Opcional: ajusta la fecha inicial del estado de cuenta
      'statement_to'     => '2023-12-31', // Opcional: ajusta la fecha final del estado de cuenta
      'attach_pdf'       => true, // Opcional: establece si se debe adjuntar el PDF
      'cc'               => 'Kakuseisha14@gmail.com', // Opcional: establece la dirección de correo electrónico CC
  ];

  // Llama a la función send_to_email con el ID de la factura y los datos de correo electrónico
  $this->send_to_email($invoiceId, $emailData);*/
...
return;
   ...
   /* Send invoice to email */
    ...
    /*   public function send_to_email($id, $emailData = [])
    ...
    /* var_dump($id);
     return;
         ...
         try {
             ...
             // Verifica si se proporcionaron datos de correo electrónico
             if (!empty($emailData)) {
                 // Accede a los datos de correo electrónico
                 $statementData['attach'] = isset($emailData['attach_statement']) ? $emailData['attach_statement'] : false;
                 $statementData['from'] = isset($emailData['statement_from']) ? to_sql_date($emailData['statement_from']) : '';
                 $statementData['to'] = isset($emailData['statement_to']) ? to_sql_date($emailData['statement_to']) : '';
             } elseif ($this->input->post('attach_statement')) {
                 // Si no se proporcionaron datos de correo electrónico, verifica los datos del formulario
                 $statementData['attach'] = true;
                 $statementData['from'] = to_sql_date($this->input->post('statement_from'));
                 $statementData['to'] = to_sql_date($this->input->post('statement_to'));
             }
...
$invoice->respuesta_data = $respuesta_data;
 //var_dump($invoice);
    ...
    public function getCajaSucursales(){
...
return; */
		...
		// get data
		$data = $this->Main_model->getCajaSucursales($postData);
