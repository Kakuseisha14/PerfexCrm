...
function get_staff_sucursal_id($userid = '')
{
    $tmpStaffUserId = get_staff_user_id();
    if ($userid == '' || $userid == $tmpStaffUserId) {
        if (isset($GLOBALS['current_user'])) {
            return $GLOBALS['current_user']->id_sucursal;
        }
        $userid = $tmpStaffUserId;
    }
    ...
    $id_sucursal = $CI->app_object_cache->get('staff-sucursal-id-' . $userid);

    if (!$id_sucursal) {
        $CI->db->where('staffid', $userid);
        $id_sucursal = $CI->db->select('id_sucursal')->from(db_prefix() . 'staff')->get()->row()->id_sucursal;
        $CI->app_object_cache->add('staff-sucursal-id-' . $userid, $id_sucursal);
    }

    return $id_sucursal;
}

function get_staff_caja_name($userid = '')
{
    $tmpStaffUserId = get_staff_user_id();
    if ($userid == '' || $userid == $tmpStaffUserId) {
        if (isset($GLOBALS['current_user'])) {
            return $GLOBALS['current_user']->caja;
        }
        $userid = $tmpStaffUserId;
    }
    ...
    $caja_name = $CI->app_object_cache->get('staff-caja-name-' . $userid);

    if (!$caja_name) {
        $CI->db->select('cajasSucursales.caja');
        $CI->db->from(db_prefix() . 'staff');
        $CI->db->join(db_prefix() . 'cajasSucursales', db_prefix() . 'staff.id_caja = ' . db_prefix() . 'cajasSucursales.id', 'left');
        $CI->db->where(db_prefix() . 'staff.staffid', $userid);

        $result = $CI->db->get()->row();

        $caja_name = $result ? $result->caja : '';
        $CI->app_object_cache->add('staff-caja-name-' . $userid, $caja_name);
    }

    return $caja_name;
}

function get_staff_caja_id($userid = '')
{
    $tmpStaffUserId = get_staff_user_id();
    if ($userid == '' || $userid == $tmpStaffUserId) {
        if (isset($GLOBALS['current_user'])) {
            return $GLOBALS['current_user']->id_caja;
        }
        $userid = $tmpStaffUserId;
    }
    ...
    $caja_id = $CI->app_object_cache->get('staff-caja-id-' . $userid);

    if (!$caja_id) {
        $CI->db->select('id_caja');
        $CI->db->from(db_prefix() . 'staff');
        $CI->db->where('staffid', $userid);

        $result = $CI->db->get()->row();

        $caja_id = $result ? $result->id_caja : '';
        $CI->app_object_cache->add('staff-caja-id-' . $userid, $caja_id);
    }

    return $caja_id;
}

//Para obtener el nombre de la caja a traves de su id
function get_caja_info($id_caja = '')
{
    $CI = &get_instance();

    $caja_info = $CI->app_object_cache->get('caja-info-' . $id_caja);

    if (!$caja_info) {
        $CI->db->select('tblcajasSucursales.caja, tblsucursal.sucursal as nombre_sucursal');
        $CI->db->from( 'tblcajasSucursales');
        $CI->db->join( 'tblsucursal',  'tblcajasSucursales.idSucursal = ' .  'tblsucursal.id', 'left');
        $CI->db->where( 'tblcajasSucursales.id', $id_caja);

        $result = $CI->db->get()->row();

        $caja_info = [
            'caja' => $result ? $result->caja : '',
            'nombre_sucursal' => $result ? $result->nombre_sucursal : '',
        ];

        $CI->app_object_cache->add('caja-info-' . $id_caja, $caja_info);
    }
    print_r($caja_info);
