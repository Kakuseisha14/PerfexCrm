...

{
...

    {
        ...
        //tablaCondicionVenta add al CRM
        public function getCustomFieldsValues($id) {
            // Verifica si la base de datos se ha cargado correctamente
            if ($this->db === NULL) {
                // Manejar un error o lanzar una excepción si la base de datos no está disponible
                // Por ejemplo: show_error('La base de datos no está disponible.');
                return NULL;
            }
    
            // Realiza la consulta
            $query = $this->db->where('relid', $id)->get('tblcustomfieldsvalues');
            ...
            if ($query) {
                ...
                // Manejar un error o lanzar una excepción si la consulta falló
                // Por ejemplo: show_error('Error al consultar la base de datos.');
                return NULL;
            }
        }

        public function getCustomFieldsValuesClient($id) {
            // Verifica si la base de datos se ha cargado correctamente
            if ($this->db === NULL) {
                // Manejar un error o lanzar una excepción si la base de datos no está disponible
                // Por ejemplo: show_error('La base de datos no está disponible.');
    ...
    public function get_invoice_items($id)
    {
        ...
        return $this->db->get(db_prefix() . 'itemable')->result();
    }
    public function getDateCrm() {
        
        $this->db->where('id', 167);
 ...
 // creation of method to register the api response
    public function responseApi($responseApi){
        $this->db->insert('tblrespuesta_api', $responseApi);
        return $this->db->insert_id();
    }

    //Creamos un get para obtener los datos guardados del api
    public function obtenerRespuestasApiPorIdFactura($idFactura) {
        // Realiza una consulta a la tabla tblrespuesta_api
        $this->db->where('idfactura', $idFactura);
        $query = $this->db->get('tblrespuesta_api');
...

    {
        ...
        // Agregar city, provincias y corregimiento si están presentes en los datos
        $data['provincia'] = isset($data['provincia']) ? $data['provincia'] : '';
        $data['distrito'] = isset($data['distrito']) ? $data['distrito'] : '';
        $data['corregimiento'] = isset($data['corregimiento']) ? $data['corregimiento'] : '';

        // Agregar city, provincias y corregimiento si están presentes en los datos
        $data['sucursal'] = isset($data['sucursal']) ? $data['sucursal'] : '';
        $data['caja'] = isset($data['caja']) ? $data['caja'] : '';
...

    {
        ...
        if (is_array($send_to) && count($send_to) > 0) {
            ...
            if ($attachpdf) {
                ...
                $pdf    = invoice_pdf($this->get($id), 'dgi');
