...

{
...

    {
        ...
        if (is_numeric($id)) {
            ...
            $this->db->where('staffid', $id);
            $this->db->join(db_prefix() . 'cajasSucursales', db_prefix() . 'cajasSucursales.id = ' . db_prefix() . 'staff.id_caja', 'left');
...

    {
        ...
        $arreglo=$data['id_sucursal'];
// Utilizando implode
$cadena_implode = implode(', ', $data['id_sucursal']);
$data['id_sucursal']=$cadena_implode;

//arreglo ahora de cajas
$arregloCajas=$data['id_caja'];
$cadena_implodeCaja= implode(', ', $data['id_caja']);
$data['id_caja']=$cadena_implodeCaja;

        $this->db->insert(db_prefix() . 'staff', $data);

        $staffid = $this->db->insert_id();
        // Asumiendo que $data['id_sucursal'] contiene un array de ids de sucursales
if (isset($arreglo) && is_array($arreglo)) {
    foreach ($arreglo as $sucursal_id) {
        $this->db->insert('staff_sucursales', [
            'staff_id' => $staffid,
...
if ($this->db->trans_status() === FALSE) {
    // Si hay algún error en alguna de las inserciones, realiza alguna acción (rollback, mensaje de error, etc.)
}
