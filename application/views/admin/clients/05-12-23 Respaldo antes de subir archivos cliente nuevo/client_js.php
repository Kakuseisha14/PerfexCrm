...
<script>
   /* $(document).ready(function () {
    // Captura el evento de cambio en el select de provincias
    $('#Provincias').change(function () {
        var provinciaId = $(this).val();

        // Obtener el token CSRF
        var csrf_token = $('[name="csrf_token"]').val(); // Ajusta según el nombre real del campo en tu formulario

        // Hacer una petición AJAX para obtener los distritos
        $.ajax({
            type: 'POST',
            url: 'admin/clients/obtener_distritos',
            data: {
                provincia_id: provinciaId,
                csrf_token: csrf_token // Agrega el token CSRF a los datos de la solicitud
            },
            success: function (data) {
                // Llena el select de distritos con los datos recibidos
                $('#Distritos').html(data);
            }
        });

        // Limpiar y deshabilitar el select de corregimientos
        $('#Corregimientos').html('').prop('disabled', true);
    });

    // Captura el evento de cambio en el select de distritos
    $('#Distritos').change(function () {
        var distritoId = $(this).val();

        // Obtener el token CSRF nuevamente si es necesario
        var csrf_token = $('[name="csrf_token"]').val();

        // Hacer una petición AJAX para obtener los corregimientos
        $.ajax({
            type: 'POST',
            url: 'obtener_corregimientos',
            data: {
                distrito_id: distritoId,
                csrf_token: csrf_token // Asegúrate de incluir el token CSRF en todas las solicitudes AJAX
            },
            success: function (data) {
                // Llena el select de corregimientos con los datos recibidos
                $('#Corregimientos').html(data).prop('disabled', false);
            }
        });
    });
});*/
