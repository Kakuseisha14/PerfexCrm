...
<div id="wrapper">
    ...
    echo ' hide';
                              ...
                              ?>
                                ...
                                <?php   //echo 'Eduar'. $member->id_sucursal;
                                $idCajas='';
                             if (isset($member)) {
                                $id_sucursal_array = explode(', ', $member->id_sucursal);

                                $id_cajas_array = explode(', ', $member->id_caja);
                                $idCajas=$member->id_caja;
                             }?>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group select-placeholder">
                                            <label for="sel_sucursal" class="control-label">Sucursal</label>
                                            <select class="selectpicker" id="sel_sucursal" name="id_sucursal[]"
                                                data-none-selected-text="-- Select city --"
                                                data-toggle="<?php echo $this->input->get('allowed_payment_modes'); ?>"
                                                data-actions-box="true" multiple="true" data-width="100%"
                            ...
                            foreach($sucursales as $sucursal): 
                                    ...
                                    ?>
                                                ...
                                                <option
                                                    <?php if (isset($member)  && in_array($sucursal['id'], $id_sucursal_array)) { echo 'selected'; } ?>
                                                    value="<?= $sucursal['id'] ?>"><?= $sucursal['sucursal'] ?></option>
                                                <?php endforeach; ?>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group select-placeholder">
                                            <label for="sel_caja" class="control-label">Caja</label>


                                            <select class="selectpicker" id="sel_caja" name="id_caja[]"
                                                data-none-selected-text="-- Select city --"
                                                data-toggle="<?php echo $this->input->get('allowed_payment_modes'); ?>"
                                                data-actions-box="true" multiple="true" data-width="100%"
                                                data-title="<?php echo _l('dropdown_non_selected_tex'); ?>">
    ...
    if (!empty($id_cajas_array)) {
        foreach ($id_cajas_array as $id_caja) {
        
            $caja_info = get_caja_info($id_caja);
            
            // Accede a la información
            $nombre_caja = $caja_info['caja'];
            $nombre_sucursal = $caja_info['nombre_sucursal'];
            ?>
                                                <option <?php if (isset($member) && in_array($id_caja, $id_cajas_array)) {
                echo 'selected';
            } ?> value="<?= $id_caja ?>"><?= $nombre_sucursal ?> -- <?= $nombre_caja ?></option>
                                                <?php
        }
    }
    ?>
    ...
    var idCaja = $('#sel_caja').val();
    idCaja = idCaja.toString();
    var idCdena = idCaja.split(', ');
    var idCdena ='<?= $idCajas ?>';
    // AJAX request
    $.ajax({
        url: '<?=base_url()?>admin/staff/getCajaSucursales',
        method: 'post',
        data: {
            sucursal: sucursal
        },
        ...
        success: function(response) {
           ...
           for (var j = 0; j < response.length; j++) {
            ...
            console.log(response[j]['id']);
                // Verificar si algún ID en idCdena coincide con el ID actual
                var selected = idCdena.includes(response[j]['id'].toString()) ? 'selected' : '';

                options += '<option value="' + response[j]['id'] + '" ' + selected + '>' + response[j]['sucursal'] + ' -- ' + response[j]['caja'] + '</option>';
            }
